...
//// COMPANION COMPUTER UDP SENDER PROGRAM 10.04.2026 ////
...
int main() {
    ...
    if (inet_pton(AF_INET, SERVER_IP, &server_addr.sin_addr) <= 0) {
        std::cerr << "Error: Invalid UDP target address." << std::endl;
        close(sockfd);
        return 1;
    }
    std::cout << "UDP Socket initialized targeting " << SERVER_IP << ":" << PORT << std::endl;
    ...
    int serial_port = open("/dev/ttyACM0", O_RDWR | O_NOCTTY | O_SYNC);
    ...
    if (ioctl(serial_port, TIOCMGET, &status) == 0) {
        ...
        std::cerr << "Warning: Failed to assert DTR/RTS." << std::endl;
    }
    ...
    std::string angle_received;
    std::string rssi_received;
    char c;
    int capturingState = 0;
    ...
    while (true) {
        if (read(serial_port, &c, 1) > 0) {
            if (c == '#') {
                ...
                angle_received = "";
                rssi_received = "";
            } else if (c == '&' && capturingState == 1) {
                capturingState = 2;
            } else if (c == '!' && capturingState == 2) {
                capturingState = 0;
                ...
                double rssi_num = 0.0;
                try {
                    rssi_num = std::stod(rssi_received);
                } catch (const std::invalid_argument& e) {
                    rssi_num = 6969.0;
                }

                std::string combined_packet = angle_received + "," + rssi_received;

                ssize_t bytes_sent = sendto(sockfd, combined_packet.c_str(), combined_packet.length(), 0,
                                        (const struct sockaddr *)&server_addr, sizeof(server_addr));
                    
                if (bytes_sent < 0) {
                    std::cerr << "Error: UDP transmit failed." << std::endl;
                }
                ...
                std::cout << "rssi: " << rssi_num << "\n";
                std::cout << "angle: " << sAngle << std::endl;
